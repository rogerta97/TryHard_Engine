...
//	assume(extremes.size() >= 3);
...

	{
		...
		std::swap(p.v[0], p.v[v0]);
		std::swap(p.v[1], p.v[v1]);
		std::swap(p.v[2], p.v[v2]);
		std::swap(p.v[3], p.v[v3]);
	}
...
//		int f = workStack[fIdx];
		...
		std::swap(workStack[fIdx], workStack.back());
		workStack.pop_back();
...
//			int vt = conflict[i];
		...
		std::swap(conflict.at(extremeCI), conflict.back());
...
//							floodFillVisited[adjFace] = floodFillVisitColor;
		...
		for(size_t i = 0; i < boundaryEdges.size(); ++i)
			for(size_t j = i+1; j < boundaryEdges.size(); ++j)
				...
				{
					std::swap(boundaryEdges[i+1], boundaryEdges[j]);
					break;
				}
#ifdef CONVEXHULL_VERBOSE
		LOGI("New boundary:");
	...
	for(size_t i = 0; i < f.size(); ++i)
	{
		...
		for(size_t j = 0; j < boundaryEdges.size(); ++j)
			for(size_t k = j+1; k < boundaryEdges.size(); ++k)
				...
				{
					std::swap(boundaryEdges[j+1], boundaryEdges[k]);
					break;
				}
		face.v.clear();
		for(size_t j = 0; j < boundaryEdges.size(); ++j)
